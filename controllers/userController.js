...
const createAccount = (req, res)=>{
    ...
    User.findOne({email: req.body.email}, (err, user)=>{
        ...
        User.create(req.body, (err, user)=>{
            if (err){
                console.log(err)                  
                return res.send("Erro while creating user");
            }  
...
const resetPassword = (req, res)=>{
    User.findOne({email: req.body.email}, (err, user)=>{
        ...
        if (!user){
            ...
            var fullUrl = (req.get("host").includes("localhost:3000") ? "http" : "https") + '://' + req.get('host') + req.originalUrl;   
            let token = jwt.sign({_id: user._id}, process.env.TOKEN_SECRET, {expiresIn: 30 * 30});
...
const resetPasswordLastStagePage = (req, res)=>{
    try{
        if (jwt.verify(req.params.token, process.env.TOKEN_SECRET)){
            return res.render("resetPasswordLastStage", {user: req.user, expired: false, reseted: false, token: req.params.token});
        }
        else{
            return res.render("resetPasswordLastStage", {user: req.user, reseted: false, expired: true});
        }
...
const resetPasswordLastStage = (req, res)=>{
    try{
        ...
        if (userVerified){
            User.findByIdAndUpdate(userVerified, {password: bcrypt.hashSync(req.body.password)}, {useFindAndModify: false}, (err, user)=>{
                if (err){
                    console.log(err);
                }
                return res.render("resetPasswordLastStage", {user: req.user, reseted: true, expired: false});
            })
        }
        else{
            return res.render("resetPasswordLastStage", {user: req.user, reseted: false, expired: true});
